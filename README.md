...
Hover or focus a sheet to preview its source page. Thumbnails warm in the background and remain cached while the project is open; repeat previews reuse the decoded image, including after renaming or calibration. Groups appear under each sheet containing their drawing objects; selecting a group inspects its sheet-local members. The inspector's **Group for new drawing** choice is separate from inspection. Drag a panel's inner edge to resize it, or use its fixed corner control to collapse, peek, and pin it. The toolbar reserves space for collapsed panel controls. Layout preferences are saved on this device. Right-click a sheet for its actions, and drag rows or use Alt-Up/Down to reorder them.
